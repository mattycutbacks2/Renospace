...
const EXAMPLE_PROMPTS = [
  ...
  "Change chair to white",
  "Paint the ceiling light blue",
  "Change countertop to marble",
  "Add white trim to the walls"
];

const ColorTouchWizard: React.FC = () => {
  ...
  const handleApplyEdit = async () => {
    ...
    try {
      ...
      // 1. Upload the image (like other tools)
      const filename = `colortouch_${Date.now()}.jpg`;
      const imagePath = await uploadImageFile(photoUri, filename);
      console.log('📤 Image uploaded:', imagePath);

      // 2. Call the FLUX tool
      console.log('🔗 Calling flux-tools with colortouch:', { prompt });
      ...
      console.log('✅ ColorTouch complete!', resultUrl);
      setProcessedImageUri(resultUrl);
      
      // Save to gallery (upload result to storage)
      try {
        const { data: { user }, error: userError } = await supabase.auth.getUser();
        if (!userError && user) {
          // Upload the result image to storage
          const resultFilename = `colortouch_result_${Date.now()}.jpg`;
        ...
        // Don't throw - the transformation worked, just the save failed
      }
      ...
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.container}>
          <View style={styles.header}>
            ...
            <Text style={styles.subtitle}>Your photo has been transformed</Text>
          </View>
          ...
          <View style={styles.footer}>
            ...
            >
              <Text style={styles.primaryButtonText}>🔄 Transform Another Photo</Text>
            </TouchableOpacity>
          </View>
        </View>
      </SafeAreaView>
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.container}>
        ...
        {/* Header */}
        <View style={styles.header}>
          <Text style={styles.title}>ColorTouch</Text>
        ...
        <ScrollView style={styles.scrollContent} contentContainerStyle={styles.scrollContentContainer}>
          ...
          {photoUri && (
            <View style={styles.promptSection}>
              <Text style={styles.sectionLabel}>Describe what you'd like to change...</Text>
              ...
              <View style={styles.promptCard}>
                <TextInput
                  ...
                  placeholder="Paint the walls navy blue..."
          ...
          >
            {isProcessing ? (
              <View style={styles.loadingContainer}>
                ...
                <Text style={styles.applyButtonText}>Transforming...</Text>
              </View>
            ) : (
              <View style={styles.applyButtonContent}>
                <Text style={styles.applyButtonIcon}>✨</Text>
                <Text style={styles.applyButtonText}>
                  {photoUri && prompt.trim() ? 'Apply Changes' : 'Upload photo and enter prompt'}
                </Text>
              </View>
            )}
          </TouchableOpacity>
...
export default ColorTouchWizard; 